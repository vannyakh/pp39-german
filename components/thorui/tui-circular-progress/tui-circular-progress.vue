		...
		methods: {
			...
			drawProgressCircular(startPercentage) {
				...
				if (!ctx) {
					...
					diam = diam * 4
					// #endif
					const progressColor = this.progressColor || (uni && uni.$tui && uni.$tui.color.primary) || '#337DFE';
				...
				if (this.fontShow) {
					...
					const fontColor = this.fontColor || (uni && uni.$tui && uni.$tui.color.primary) || '#337DFE';
