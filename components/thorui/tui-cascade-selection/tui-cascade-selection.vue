...
<script>
	export default {
		...
		computed: {
			getActiveColor() {
				return this.activeColor || (uni && uni.$tui && uni.$tui.color.primary) || '#337DFE';
			},
			getLineColor(){
				return this.lineColor || (uni && uni.$tui && uni.$tui.color.primary) || '#337DFE';
			},
			getCkMarkColor(){
				return this.checkMarkColor || (uni && uni.$tui && uni.$tui.color.primary) || '#337DFE';
			}
		},
		watch: {
			itemList(val) {
