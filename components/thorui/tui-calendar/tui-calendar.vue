	...
	<view @touchmove.stop.prevent="stop" v-if="isFixed">
		<view class="tui-bottom-popup" :class="{'tui-popup-show': isShow}">
			...
			<view class="tui-date-box">
				...
				<view class="tui-date_time">{{ showTitle | Thai }}</view>
			...
			<view class="tui-date-header">
				<view class="tui-date">{{ $t('calendar1') }}</view>
				<view class="tui-date">{{ $t('calendar2') }}</view>
				<view class="tui-date">{{ $t('calendar3') }}</view>
				<view class="tui-date">{{ $t('calendar4') }}</view>
				<view class="tui-date">{{ $t('calendar5') }}</view>
				<view class="tui-date">{{ $t('calendar6') }}</view>
				<view class="tui-date">{{ $t('calendar7') }}</view>
			</view>
			...
			<view class="tui-calendar-op">
				<view class="tui-calendar-result">
					<text>{{ type == 1 ? activeDate : startDate | Thai}}</text>
					<text v-if="endDate">{{ $t('calendar20') }}{{ endDate | Thai}}</text>
				</view>
				<view class="tui-calendar-btn_box">
					<view :style="{background:getBtnBgColor}" @tap.stop="btnFix(false)" class="tui-btn__confirm"
						:class="{'tui-btn__disabled':disabled,'tui-btn__confirm-hover':!disabled}">{{ $t('calendar24') }}</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		computed: {
			...
			getActiveBgColor(){
				return this.activeBgColor || (uni && uni.$tui && uni.$tui.color.primary) || '#337DFE';
			},
			getBtnBgColor() {
				return this.btnBgColor || (uni && uni.$tui && uni.$tui.color.primary) || '#337DFE';
			},
			getRangeColor(){
				return this.rangeColor || (uni && uni.$tui && uni.$tui.color.primary) || '#337DFE';
			}
		},
		watch: {
			dataChange(val) {
