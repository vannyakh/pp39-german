...
import { translate as $t } from "@/main"
	...
	if(data.updateType == 'forcibly' || data.updateType == 'solicit'){
		...
		dtask = plus.downloader.createDownload(data.downloadUrl, {
			...
			if (status == 200) {
				popupObj.change({
					progressValue: 100,
					progressTip: $t('app_anzhuan'),
					progress: true,
					buttonNum: 0
				});
				plus.runtime.install(download.filename, {}, function() {
					popupObj.change({
						contentText: $t('app_wancheng'),
						buttonNum: 1,
						progress: false
					});
				}, function(e) {
				...
				case 3:
					...
					if(progress - lastProgressValue >= 2){
						...
						popupObj.change({
							...
							progressTip: $t('app_download') + progress + "%",
							progress: true
						});
					}
					break;
...
function updatePopup(data, callback) {
	...
	let popupViewContentList = [{
		...
		},
		{
			tag: 'font',
			id: 'title',
			text: $t('app_new') + data.versionName,
			textStyles: {
				size: '18px',
				color: "#333",
				weight: "bold",
...
function downloadPopupDrawing(data){
	...
	let progressTip = data.progressTip || $t('app_randd');
	let contentText = data.contentText || $t('app_load');
	let elementList = [
		{
			tag: 'rect', //背景色
			color: '#FFFFFF',
