<template>
	<view class="position-card" :style="{ background: theme.backgroundCard }">
		...
		<view class="position-details">
			<view class="detail-section">
				<view class="detail-labels">
					<text class="detail-label" :style="{ color: theme.textSecondary }">{{$t('Current_Price')}}</text>
					<text class="detail-label" :style="{ color: theme.textSecondary }">/</text>
					<text class="detail-label" :style="{ color: theme.textSecondary }">{{$t('Cost')}}</text>
				</view>
				<text class="detail-value" :style="{ color: theme.textPrimary }">{{item.close}}/{{item.price}}</text>
			</view>

			<view class="detail-section">
				<text class="detail-label" :style="{ color: theme.textSecondary }">{{$t('Lot')}}</text>
				<text class="detail-value" :style="{ color: theme.textPrimary }">{{item.num}}</text>
			</view>

			<view class="detail-section">
				<text class="detail-label" :style="{ color: theme.textSecondary }">{{$t('P&L')}}</text>
		...
		<view class="position-footer">
			<view class="detail-section">
				<text class="detail-label" :style="{ color: theme.textSecondary }">{{$t('Position_ratio')}}</text>
			...
			<view class="detail-section">
				<text class="detail-label" :style="{ color: theme.textSecondary }">{{$t('Total_Purchased')}}</text>
			...
			<view v-if="status == 2" @click="handleSell" class="action-section">
				<view class="action-button sell-button" :style="{ background: theme.primary__light, color: theme.white }">{{$t('Sell')}}</view>
			</view>

			<view v-if="status==3" @click="handleDetail" class="action-section">
				<view class="action-button detail-button" :style="{ background: theme.success__500, color: theme.white }">{{$t('position5')}}</view>
			</view>
		</view>
	</view>
</template>
