<template>
	<view :style="{background: theme.light,minHeight: '100vh',  color: theme.white}">
		<z-paging :language="$t('locale')" ref="paging" v-model="list" @query="queryList" :pageSize="20" :auto="false">
			<myNav slot="top" :title="$t('tabBar3')" :black="false" isRight righturl="/static/imgs/index/soushuo.png"
				@Jump="router.push('/pages/index/searchView')"></myNav>
				...
				<view class="padding-w-20 padding-c-20 radiu-30" :style="{background: theme.light__gray}">
					<view  class="padding-w-10 padding-c-10 radiu-20 border-05-BCC7BF" >
					    <view class="radiu-10 text-center flex align-center justify-between"
					        ...
					        <view class="width-100b radiu-15 flex align-center justify-center height-80 bold-font"
					...
					<view  class="padding-w-2 padding-c-10" >
					    <view class=" text-center flex align-center justify-between">
					        <view class="width-100b  flex direction-column align-center justify-center height-150 radiu-20 border-05-BCC7BF" :style="{background: theme.BC_BCC7BF}" >
					        ...
					        <view class="width-100b  flex direction-column align-center justify-center height-150 radiu-20 border-05-BCC7BF" :style="{background: theme.BC_BCC7BF}">
					            ...
					            <view class="height-50 text-center font-28 bold-font" :style="{color:theme.green__light}">¥ {{Number(money_cn?money_cn:0).toFixed(3)}}</view>
					        </view>
					    </view>
					</view>
					<view class="flex align-center  radiu-30 justify-between width-100b border-05-BCC7BF"
						:style="{background: theme.BC_BCC7BF}"  >
						<view class="width-100b" v-for="(item,index) in categoryList" :key="index">
							<view @click="set(item.type)" class="text-center font-25 radiu-15 padding-20"
				...
				<view class="listBox":style="{background: theme.light__gray}" v-for="(item, index) in list" :key="index">
					...
					<view class="flex align-center justify-between">
						...
						<view class="flex flex-direction text-center flex-1 gap-15">
							...
							<view class="font-20"
								:style="{color: item.rise_all_fall >= 0 ?theme.green__dark:theme.danger}">
					...
					<view class="">
						<view v-if="status == 2" @click="showPopup(item)"
							class="flex align-center justify-between relative margin-t-20 custom">
							<view class="padding-10 font-25 width-100b"
								:style="{borderRadius:'20rpx 0 0 20rpx', background:Number((item.close-item.price)/item.price*100).toFixed(3)>=0?theme.green__dark:theme.danger, color:theme.white}">
								{{$t('optional7')}} <span
									class="margin-l-10">{{Number((item.close-item.price)/item.price*100).toFixed(3)}}%</span>
							</view>
							<view class="padding-10 font-25 width-100b text-center border-05-BCC7BF"
								:style="{borderRadius:'0 20rpx 20rpx 0', background:theme.light__gray, color:theme.dark}">
								<text>{{$t('chicang11')}}</text>
							</view>
						</view>
						<view v-if="status == 3" @click="showDetail(item.id)"
							class="flex align-center justify-between relative margin-t-20 custom ">
							<view class="padding-15 font-20 width-100b"
								:style="{borderRadius:'20rpx 0 0 20rpx', background:Number((item.pc_price-item.price)/item.price*100).toFixed(3)>=0?theme.green__dark:theme.danger, color:theme.white}">
								{{$t('optional7')}} <span
									class="margin-l-10">{{Number((item.pc_price-item.price)/item.price*100).toFixed(3)}}%</span>
							</view>

							<view class="padding-15 font-20 width-100b text-right "  style="border: solid #353E4B 1px;"
								:style="{borderRadius:'0 20rpx 20rpx 0', background:theme.light__gray, color:theme.dark}">
								<text>{{$t('chicang11')}}</text>
							</view>
						</view>
		...
		<u-popup :show="show" :round="20" mode="center" bgColor="transparent" @close="show=false" @open="open">
			<view class="height-370 width-690 radiu-40" :style="{background: theme.light__gray,  color: theme.dark}">
				<view class="height-220 flex direction-column align-center justify-center">
					...
					<view v-if="country==6"  class="font-48 margin-t-30"> CA$ {{Number(win_or_lost).toFixed(3)}}</view>
				...
				<view class="height-150 flex align-center justify-around">
					...
					<view @click="pingcang(id)" class="padding-20  radiu-10"
