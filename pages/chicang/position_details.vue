<template>
	<view class="position-details-page min-height-screen" :style="{ background: theme.backgroundSecondary, color: theme.textPrimary }">
        <bg-animate-shape/>
		<app-nav :title="$t('PositionDetails1')" blur></app-nav>
			...
			<view v-else class="position-header-card" :style="{ background: theme.backgroundCard, borderColor: theme.neutral__200 }">
				...
				<view class="status-section">
					...
					<view class="percentage-badge" :style="{ color: theme.danger__500 }">
						<span v-if="dataObj.status==1">{{$t('PositionDetails5')}}</span>
				...
				<view class="price-lot-section" :style="{ background: theme.backgroundTertiary }">
					<view class="info-item">
						<text class="info-label" :style="{ color: theme.textSecondary }">{{$t('PositionDetails6')}}</text>
						<text class="info-value" :style="{ color: theme.textPrimary }">{{dataObj.price}}</text>
					</view>
					<view class="info-item">
						<text class="info-label" :style="{ color: theme.textSecondary }">{{$t('Lot')}}</text>
		...
		<v-container size="md">
			...
			<view v-else class="basic-info-card" :style="{ background: theme.backgroundCard, borderColor: theme.neutral__200 }">
				<view class="card-header">
					<text class="card-title" :style="{ color: theme.textPrimary }">{{$t('PositionDetails8')}}</text>
				</view>
				<view class="info-row">
					<text class="info-label" :style="{ color: theme.textSecondary }">{{$t('PositionDetails9')}}</text>
					<text class="info-value" :style="{ color: theme.textPrimary }">{{dataObj.k_or_d=='k' ? $t('PositionDetails10') : $t('PositionDetails11')}}</text>
				</view>
				<view class="info-row">
					<text class="info-label" :style="{ color: theme.textSecondary }">{{$t('PositionDetails12')}}</text>
					<text class="info-value" :style="{ color: theme.textPrimary }">{{dataObj.createtime}}</text>
				</view>
				<view v-if="dataObj.status==3" class="info-row">
					<text class="info-label" :style="{ color: theme.textSecondary }">{{$t('PositionDetails13')}}</text>
					<text class="info-value" :style="{ color: theme.textPrimary }">{{dataObj.updatetime}}</text>
				</view>
				<view v-if="dataObj.status==3" class="info-row">
					<text class="info-label" :style="{ color: theme.textSecondary }">{{$t('PositionDetails14')}}</text>
					<text class="info-value" :style="{ color: theme.textPrimary }">{{dataObj.pc_price}}</text>
				</view>
				<view class="info-row">
					<text class="info-label" :style="{ color: theme.textSecondary }">{{$t('PositionDetails15')}}</text>
					<text class="info-value" :style="{ color: theme.textPrimary }">{{calculateMargin(dataObj.close, dataObj.num)}}</text>
				</view>
				<view class="info-row">
					<text class="info-label" :style="{ color: theme.textSecondary }">{{$t('PositionDetails16')}}</text>
					<text class="info-value" :style="{ color: theme.textPrimary }">{{dataObj.leverage || 1}}</text>
				</view>
				<view class="info-row">
					<text class="info-label" :style="{ color: theme.textSecondary }">{{$t('PositionDetails17')}}</text>
					<text class="info-value" :style="{ color: theme.textPrimary }">{{dataObj.close}}</text>
				</view>
				<view class="info-row">
					<text class="info-label" :style="{ color: theme.textSecondary }">{{$t('PositionDetails18')}}</text>
					<text class="info-value" :style="{ color: theme.textPrimary }">{{dataObj.type==1 ? $t('PositionDetails19') : $t('PositionDetails20')}}</text>
				</view>
				<view class="info-row">
					<text class="info-label" :style="{ color: theme.textSecondary }">{{$t('position2')}}</text>
					<text class="info-value" :style="{ color: getPnLColor(dataObj.rise_all_fall) }">
						{{formatNumber(dataObj.rise_all_fall)}}
					</text>
				</view>
				<view class="info-row">
					<text class="info-label" :style="{ color: theme.textSecondary }">{{$t('PositionDetails21')}}</text>
	...
	export default {
		...
		methods: {
			...
			getdetails(id,status) {
				this.isLoading = true
				this.post('/shares/order_list_id', {
					id: id,
					status:status
				}).then((res) => {
					if (res.code == 1) {
