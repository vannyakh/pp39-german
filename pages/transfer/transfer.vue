<template>
    <view class="transfer-container" :style="{background: theme.backgroundPrimary, color: theme.textPrimary}">
        <!-- Header -->
        <bg-animate-shape/>
        <app-nav blur :title="$t('transfer.title')" showRightImage
                 rightImageUrl="/static/imgs/fund/record.png"
                 @right-action="router.push('/pages/transfer/record')"
        ></app-nav>
                ...
                <view class="transfer-direction">
                    ...
                    <view class="direction-item">
                        ...
                        <view class="direction-value" @click="openFromSelector">
                            <text class="direction-text" :class="{ 'swapping': isSwapping }" :style="{color: theme.textPrimary}">
                                {{ fromBalance.type === 'money' ? 'Trading' : (fromBalance.type === 'spot' ? 'Spot' : $t('transfer.selectWallet')) }}</text>
                    ...
                    <view class="direction-item">
                        ...
                        <view class="direction-value" @click="openToSelector">
                            <text class="direction-text" :class="{ 'swapping': isSwapping }" :style="{color: theme.textPrimary}">{{ toBalance.type === 'money' ? 'Trading' : (toBalance.type === 'spot' ? 'Spot' : $t('transfer.selectWallet')) }}</text>
                            <text class="cuIcon-right" :style="{color: theme.textTertiary}" />
                        </view>
                    </view>
                </view>
                ...
                <view class="coin-selector" @click="openAssetSelector" :style="{background: theme.backgroundCard, border: `2rpx solid ${theme.neutral__200}`}">
                    <view class="coin-info">
                        ...
                        <view class="coin-details">
                            ...
                            <text class="coin-full-name" :style="{color: theme.textSecondary}">{{ selectedAsset.name ? selectedAsset.currency : $t('transfer.asset.select') }}</text>
                        </view>
                    </view>
                    <text class="cuIcon-right" :style="{color: theme.textTertiary}" />
                </view>
            ...
            <view class="amount-section">
                ...
                <view class="amount-input-container" :style="{background: theme.backgroundCard, border: `2rpx solid ${theme.neutral__200}`}">
                    ...
                    <view class="amount-input">
                        <input v-model="transferAmount" type="number" :placeholder="$t('transfer.enterAmount')" class="amount-field" :style="{color: theme.textPrimary}" placeholder-class="input-placeholder" />
                    </view>
                    <view class="available-balance">
                        <text class="available-text" :style="{color: theme.textTertiary}">{{ $t('transfer.available', { amount: formatNumber(availableBalance), currency: currency }) }}</text>
                    </view>
                </view>
            </view>
        </v-container>
        ...
        <view class="bottom-section">
            <v-container size="md">
                <u-button :disabled="!isTransferValid" @click="confirmTransfer" class="confirm-button" :block="true"
                          shape="circle" size="large" :text="isTransferValid ? $t('transfer.confirmTransfer') : $t('transfer.insufficientBalance')"
        ...
        <v-bottom-sheet v-model="showAccountSelector" :close-on-backdrop="true"
                        :title="$t('transfer.bottomSheet.title')"
        ...
        <v-bottom-sheet v-model="showAssetSelector" :close-on-backdrop="true"
                        :title="$t('transfer.asset.select')"
                        :backgroundColor="theme.backgroundCard">
            <view class="asset-list">
                <view v-for="asset in assetTypes" :key="asset.id" class="asset-item"
...
export default {
    ...
    computed: {
        ...
        accountOptions() {
            const out = [];
            (this.accountData.spot_list || []).forEach(i => out.push({
                compositeId: `spot-${i.id}`,
                id: i.id,
                type: 'spot',
                name: i.wallet
            }));
            (this.accountData.money_list || []).forEach(i => out.push({
    ...
    methods: {
        ...
        updateAssetTypes() {
            ...
            const spotList = (this.accountData.spot_list || []).map(i => ({ id: i.name, name: i.name, currency: i.currency, symbol: i.symbol, icon: this.getIconForAsset(i.name) }));
            const moneyList = (this.accountData.money_list || []).map(i => ({ id: i.name, name: i.name, currency: i.currency, symbol: i.symbol, icon: this.getIconForAsset(i.name) }));
        ...
        getBalanceKey(type, assetLower) {
            ...
            if (type === 'spot') {
                if (assetLower === 'cnd' || assetLower === 'cad') return 'spot_money_cnd';
                ...
                if (assetLower === 'cnd' || assetLower === 'cad') return 'money_cnd';
